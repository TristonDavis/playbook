...
const tiers = [
  {
    name: 'Free',
    price: '$0',
    desc: 'Everything you need to get started tracking your picks.',
    featured: false,
  ...
  },
  {
    name: 'Pro',
    price: '$12',
    desc: 'For analysts who track seriously across multiple sports.',
    featured: true,
    popular: true,
  ...
  },
  {
    name: 'Elite',
    price: '$29',
    desc: 'For power users who want every edge and export capability.',
    featured: false,
    cta: 'Get Elite',
    ctaHref: '/auth/sign-up',
    ...
    <div className="min-h-screen bg-pb-bg text-pb-text font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-pb-bg/95 backdrop-blur border-b border-pb-border h-14 flex items-center justify-between px-8">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <a href="#sports" className="text-[13px] text-pb-muted hover:text-pb-text transition-colors">Sports</a>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/auth/sign-in" className="text-[13px] font-medium text-pb-muted px-3.5 py-1.5 rounded-lg border border-pb-border2 hover:bg-pb-surface hover:text-pb-text transition-colors">
            Log in
          </Link>
          <Link href="/auth/sign-up" className="text-[13px] font-medium text-white px-4 py-1.5 rounded-lg bg-pb-blue hover:bg-blue-700 transition-colors">
            Start free
          </Link>
        </div>
      </nav>
      ...
      <div className="flex justify-center px-6 py-10 border-b border-pb-border bg-pb-bg">
        <div className="w-full max-w-2xl bg-pb-surface border border-pb-border2 rounded-2xl overflow-hidden">
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            {/* Prediction rows */}
            {[
              { sport: 'bg-pb-blue', title: 'Chiefs -3.5 vs Raiders', meta: 'NFL · Sun 3:25 PM ET · 2u', badge: 'Win', badgeStyle: 'bg-green-500/10 text-green-400 border border-green-500/20' },
              { sport: 'bg-amber-400', title: 'Lakers ML vs Clippers', meta: 'NBA · Tonight 10:00 PM ET · 1u', badge: 'Pending', badgeStyle: 'bg-amber-400/10 text-yellow-300 border border-amber-400/20' },
              { sport: 'bg-green-500', title: 'Yankees O7.5 vs Red Sox', meta: 'MLB · Sat 1:05 PM ET · 1.5u', badge: 'Loss', badgeStyle: 'bg-red-500/10 text-red-400 border border-red-500/20' },
            ].map((p) => (
              <div key={p.title} className="bg-pb-surface2 border border-pb-border rounded-lg px-3 py-2.5 flex items-center gap-2.5">
                ...
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${p.badgeStyle}`}>{p.badge}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {tiers.map((t) => (
            <div
              ...
              className={`rounded-2xl p-6 flex flex-col gap-4 ${
                t.featured
                  ? 'bg-[#0D1525] border border-blue-400/30'
                  : 'bg-pb-bg border border-pb-border'
              }`}
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-[13px] font-medium uppercase tracking-wider ${t.featured ? 'text-pb-blue-lt' : 'text-pb-muted'}`}>
                    {t.name}
                  </span>
                  {t.popular && (
                    <span className="text-[10px] font-medium text-pb-blue-lt bg-blue-500/15 border border-blue-400/20 px-2 py-0.5 rounded-full">
                      Most popular
                    </span>
                  )}
                </div>
              ...
              <Link
                href={t.ctaHref}
                className={`mt-auto w-full text-center py-2.5 rounded-lg text-[13px] font-medium transition-colors ${t.ctaStyle}`}
              >
                {t.cta}
              </Link>
            </div>
      ...
      <section className="flex flex-col items-center gap-4 text-center px-8 py-20 bg-pb-bg">
        ...
        <p className="text-[14px] text-pb-muted max-w-sm leading-relaxed">
          Join analysts using The Playbook to build an edge. Free to start, no credit card required.
        </p>
      ...
      <footer className="bg-pb-surface border-t border-pb-border px-8 py-6 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <LogoMark size={20} />
          <span className="text-[13px] font-medium text-pb-muted">
            The <span className="text-pb-blue-lt">Playbook</span>
          </span>
        </div>
